...
clone_kernel() {
    if [ -d ${KERNEL_PARENT_DIR} ]; then
        ...
        git checkout linux/${KERNEL_BRANCH}
    else
        mkdir -p ${KERNEL_PARENT_DIR}
        echo Created the kernel target directory ${KERNEL_PARENT_DIR}... cloning ...zzz.
...
make -j${NCORES} CROSS_COMPILE=${CC} ${KERNEL_CONF} NAME="Michael Brown" EMAIL="producer@holotronic.dk" ARCH=arm KBUILD_DEBARCH=armhf LOCALVERSION=-${KERNEL_LOCALVERSION} KDEB_PKGVERSION=${KERNEL_VERSION}-${KERNEL_REV} deb-pkg 2>&1 | tee ../deb_rt-log.txt
